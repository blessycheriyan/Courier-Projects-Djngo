...
def payout_to_courier(modeladmin, request, queryset):
  ...
  # Step 1 - Get all the valid couriers (w/ valid email address) in queryset
...
class CourierAdmin(admin.ModelAdmin):
  ...
  def balance(self, obj):
    return round(sum(t.amount for t in Transaction.objects.filter(job__courier=obj, status=Transaction.IN_STATUS)) * 0.7, 2)
